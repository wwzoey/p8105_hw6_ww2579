...
The distribution of r^2 seems to have a mean of `r mean(log_df$r.squared)`, which is a quite high coefficient of determination, indicating on average, the model fits well on data. It can also be observed that there is a longer tail on the left side of the curve, which may be related to the frequency with which large outliers are included in bootstrap samples.

The distribution of log product of beta0 and beta1 is approximately normal, with a mean of `r mean(log_df$log_estimates)`.
